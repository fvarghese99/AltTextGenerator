...
def test_update_alt_text_no_alt_text(monkeypatch, tmpdir):
    ...
    alt_text = "   "  # After sanitization, this should be empty
...
def test_main_with_images(mock_input, mock_find_images, mock_load_blip2_model, mock_generate_caption,
                          mock_update_alt_text):
    mock_find_images.return_value = ["/path/to/images/image1.jpg", "/path/to/images/image2.png"]
    mock_processor = MagicMock()
    mock_model = MagicMock()
    mock_device = torch.device("cpu")  # Now torch is defined
    ...
    with patch('builtins.print') as mock_print:
        ...
        assert mock_generate_caption.call_count == 2
        assert mock_update_alt_text.call_count == 2
        # Ensure update_alt_text is called with correct parameters
